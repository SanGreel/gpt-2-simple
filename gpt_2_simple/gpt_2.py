...
from gpt_2_simple_colab.src import model, sample, encoder, memory_saving_gradients
from gpt_2_simple_colab.src.load_dataset import load_dataset, Sampler
from gpt_2_simple_colab.src.accumulate import AccumulatingOptimizer
...
def finetune(
    ...
    def validation():
        ...
        summary_log.flush()
        print(
            "[{counter} | {time:2.2f}] validation loss = {loss:2.4f}".format(
    ...
    try:
        while True:
            ...
            if val_every > 0 and (counter % val_every == 0 or counter == 1):
                ...
                print(
                    "[{counter} | {time:2.2f}] prev_val_loss={val_score:2.4f} new_val_loss={val_score_new:2.4f}".format(
                        counter=counter,
                ...
                # round(val_score, 3) < 2.35
                if (
                    (val_score_new) > (val_score+val_error_acceptable)
                    and val_score_new > 0
                    and val_score > 0
                    and counter > 1
                ):
            ...
            if counter % print_every == 0:
                ...
                print(
                    "[{counter} | {time:2.2f}] loss={loss:2.4f} avg={avg:2.4f}".format(
                        counter=counter,
                        time=time.time() - start_time,
                        loss=v_loss,
...
def copy_folder_from_gdrive(file_path):
    """Copies a file from a mounted Google Drive."""
    is_mounted()
